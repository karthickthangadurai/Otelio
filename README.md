Here's a draft — written in plain human voice, covering everything real so far. Note that I've marked the unbuilt parts as `TODO` rather than describing them as done; fill those in as you build, and delete the markers before submitting.
...
Two notes on using this. First, the "Key design decisions" section is the strongest part of your submission and it's *already written* — because you actually made those decisions, in order, with reasons. Second, notice how much of the file is `TODO`: everything downstream of the reservation layer. That's an accurate picture of your repo right now.

The README is done as far as it can be. The remaining TODOs get filled by *building*, not writing. Same ask as it's been: run that ten-line `create_reservation` and paste what prints.